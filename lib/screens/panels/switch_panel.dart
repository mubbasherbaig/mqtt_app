...
class _AddSwitchPanelScreenState extends State<AddSwitchPanelScreen> {
  ...
  @override
  void dispose() {
    _panelNameController.dispose(); _topicController.dispose();
    _subscribeTopicController.dispose(); _payloadOnController.dispose();
    _payloadOffController.dispose(); super.dispose();
  }
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Wrap(spacing: 10, runSpacing: 10, children: colors.map((c) => GestureDetector(
        ...
        child: Container(width: 40, height: 40, decoration: BoxDecoration(color: c, shape: BoxShape.circle,
            border: Border.all(color: _switchColor == c ? Colors.black : Colors.transparent, width: 3))),
      )).toList()),
    ));
  }
  ...
  Widget _fieldRow(String label, TextEditingController ctrl, {bool required = false, bool showHelp = false, Widget? trailingIcon, String? Function(String?)? validator}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 0), child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
        ...
        if (trailingIcon != null) Padding(padding: const EdgeInsets.only(left: 10, bottom: 8), child: trailingIcon),
        if (showHelp && trailingIcon == null) Padding(padding: const EdgeInsets.only(left: 10, bottom: 8), child: _helpIcon()),
      ])),
      _divider(),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.white, elevation: 0,
          ...
          title: Text(l.addSwitchPanel, style: const TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
      ...
      body: Form(key: _formKey, child: ListView(children: [
        _fieldRow(l.panelName, _panelNameController, required: true,
        ...
        _fieldRow(l.subscribeTopic, _subscribeTopicController, showHelp: true),
        _fieldRow(l.payloadOn, _payloadOnController, required: true, validator: (v) => (v==null||v.isEmpty) ? l.required : null),
        _fieldRow(l.payloadOff, _payloadOffController, required: true, validator: (v) => (v==null||v.isEmpty) ? l.required : null),
        // Switch color
        Column(children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            GestureDetector(onTap: _pickColor, child: Container(width: 110, height: 36,
                decoration: BoxDecoration(color: _switchColor, borderRadius: BorderRadius.circular(4)),
