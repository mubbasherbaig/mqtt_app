...
class AddRadioButtonsPanelScreen extends StatefulWidget {
  ...
  @override
  State<AddRadioButtonsPanelScreen> createState() => _AddRadioButtonsPanelScreenState();
}

class _AddRadioButtonsPanelScreenState extends State<AddRadioButtonsPanelScreen> {
  ...
  void _addItem() => _items.add({'label': TextEditingController(), 'payload': TextEditingController()});

  @override
  void dispose() {
    _panelNameCtrl.dispose(); _topicCtrl.dispose(); _subscribeTopicCtrl.dispose();
    for (final i in _items) { i['label']!.dispose(); i['payload']!.dispose(); }
    super.dispose();
  }
  ...
  void _create() {
    if (_formKey.currentState!.validate()) {
      Navigator.pop(context, {
        'type': 'Radio Buttons', 'label': _panelNameCtrl.text.trim(),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.white, elevation: 0,
          ...
          title: const Text('Add a Radio Buttons panel', style: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
      ...
      body: Form(key: _formKey, child: ListView(children: [
        ...
        ...List.generate(_items.length, (i) => Column(children: [
          _fieldRow('Label for item ${i+1}', _items[i]['label']!, required: true, validator: (v) => (v==null||v.isEmpty) ? 'Required' : null),
          _fieldRow('Payload for item ${i+1}', _items[i]['payload']!, required: true, validator: (v) => (v==null||v.isEmpty) ? 'Required' : null),
        ])),
        Column(children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Add radio item', style: TextStyle(fontSize: 15, color: Colors.black87)),
