...
class _AddChartPanelScreenState extends State<AddChartPanelScreen> {
  ...
  final List<String> _chartTypes = ['Pie chart', 'Donut chart', 'Bar chart'];
  final List<int>    _qosOptions = [0, 1, 2];
  final List<Color>  _defaultColors = [const Color(0xFFEA1111), const Color(0xFF2FCB11), const Color(0xFF1E88E5), Colors.orange, Colors.purple];

  final List<Map<String, dynamic>> _items = [];

  @override
  void initState() { super.initState(); _addItem(); _addItem(); }
  ...
  void _pickColor(int idx) {
    final colors = [const Color(0xFFEA1111), const Color(0xFF2FCB11), const Color(0xFF1E88E5), Colors.orange, Colors.purple, Colors.teal, Colors.pink, Colors.indigo];
    showDialog(context: context, builder: (_) => AlertDialog(
      title: const Text('Pick Chart Color'),
      content: Wrap(spacing: 10, runSpacing: 10, children: colors.map((c) => GestureDetector(
        onTap: () { setState(() => _items[idx]['color'] = c); Navigator.pop(context); },
        child: Container(width: 40, height: 40, decoration: BoxDecoration(color: c, shape: BoxShape.circle,
            border: Border.all(color: (_items[idx]['color'] as Color) == c ? Colors.black : Colors.transparent, width: 3))),
      )).toList()),
    ));
  }
  ...
  void _create() {
    if (_formKey.currentState!.validate()) {
      Navigator.pop(context, {
        'type': 'Chart', 'label': _panelNameCtrl.text.trim(),
        'chartType': _chartType, 'unit': _unitCtrl.text.trim(),
        'items': _items.map((i) => {
  ...
  Widget _itemBlock(int idx) {
    final item = _items[idx];
    final color = item['color'] as Color;
    final hex = '#${color.value.toRadixString(16).substring(2).toUpperCase()}';
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _field('Topic for item ${idx + 1}', item['topic'] as TextEditingController, req: true, val: (v) => (v==null||v.isEmpty) ? 'Required' : null),
      _field('Label for item ${idx + 1}', item['label'] as TextEditingController),
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 0), child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            TextFormField(controller: item['factor'] as TextEditingController, keyboardType: TextInputType.number, style: const TextStyle(fontSize: 15),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            TextFormField(controller: item['decimalPrecision'] as TextEditingController, keyboardType: TextInputType.number, style: const TextStyle(fontSize: 15),
                decoration: const InputDecoration(isDense: true, contentPadding: EdgeInsets.only(top: 4, bottom: 8),
                    enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: Colors.black26)),
                    focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: Color(0xFF1E88E5))))),
          ])),
      ...
      Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(16, 14, 16, 10), child: Row(children: [
          GestureDetector(onTap: () => _pickColor(idx),
              child: Container(width: 36, height: 36, decoration: BoxDecoration(color: color, shape: BoxShape.circle))),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Chart color', style: TextStyle(fontSize: 13, color: Colors.black54)),
      ...
      _check('Enable notification or alarm', item['enableNotif'] as bool, (v) => setState(() => item['enableNotif'] = v), help: true, enabled: false),
      _check('Payload is JSON Data', item['payloadIsJson'] as bool, (v) => setState(() => item['payloadIsJson'] = v)),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.white, elevation: 0,
          ...
          title: const Text('Add a Chart panel', style: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.w600)),
      ...
      body: Form(key: _formKey, child: ListView(children: [
        ...
        ...List.generate(_items.length, (i) => _itemBlock(i)),
        Column(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: const EdgeInsets.fromLTRB(16, 18, 16, 0), child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            const SizedBox(width: 24),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Chart type', style: TextStyle(fontSize: 13, color: Colors.black54)),
              DropdownButtonFormField<String>(value: _chartType, style: const TextStyle(fontSize: 15, color: Colors.black87),
                  decoration: const InputDecoration(isDense: true, contentPadding: EdgeInsets.only(top: 4, bottom: 8),
                      enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: Colors.black26))),
                  items: _chartTypes.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
