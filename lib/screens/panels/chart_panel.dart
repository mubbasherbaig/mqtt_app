...
class AddChartPanelScreen extends StatefulWidget {
  const AddChartPanelScreen({super.key, this.initialData});
...
class _AddChartPanelScreenState extends State<AddChartPanelScreen> {
  ...
  void initState() {
    ...
    if (widget.initialData != null) {
      ...
      _chartType = d['chartType'] as String? ?? 'Pie chart';
        ...
        for (int i = 0; i < savedItems.length; i++) {
          ...
          _items.add({
            ...
            'factor': TextEditingController(
              text: item['factor']?.toString() ?? '1',
            ),
            'decimalPrecision': TextEditingController(
              text: item['decimalPrecision']?.toString() ?? '0',
            ),
            'color': colorVal != null
                ? Color(colorVal)
                ...
                (c) => GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: (_items[idx]['color'] as Color) == c
                            ? Colors.black
                            : Colors.transparent,
                        width: 3,
                      ),
        ...
        'items': _items
            .map(
              (i) => {
                ...
                'decimalPrecision':
                    (i['decimalPrecision'] as TextEditingController).text
                        .trim(),
  ...
  Widget _itemBlock(int idx, AppLocalizations l) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          _field(
            '${l.topic} (${l.item} ${idx + 1})',
            item['topic'] as TextEditingController,
            req: true,
            val: (v) => (v == null || v.isEmpty) ? l.required : null,
          ),
          _field(
            '${l.label} (${l.item} ${idx + 1})',
            item['label'] as TextEditingController,
          ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        controller:
                            item['decimalPrecision'] as TextEditingController,
                        keyboardType: TextInputType.number,
                        decoration: const InputDecoration(
                          isDense: true,
                          contentPadding: EdgeInsets.symmetric(vertical: 8),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _isEditing ? l.edit : l.addChartPanel,
          style: const TextStyle(
            color: Colors.black,
            fontSize: 18,
