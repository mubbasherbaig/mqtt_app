...
class _AddGaugePanelScreenState extends State<AddGaugePanelScreen> {
  ...
  @override
  void dispose() {
    _panelNameCtrl.dispose(); _topicCtrl.dispose(); _payloadMinCtrl.dispose();
    _payloadMaxCtrl.dispose(); _factorCtrl.dispose(); _decimalPrecisionCtrl.dispose();
    _unitCtrl.dispose(); _threshold1Ctrl.dispose(); _threshold2Ctrl.dispose();
    super.dispose();
  }

  void _pickArcColor(int index) {
    final colors = [Colors.green, Colors.yellow, Colors.orange, Colors.red, const Color(0xFF1E88E5), Colors.purple, Colors.teal, Colors.pink];
    showDialog(context: context, builder: (_) => AlertDialog(
      title: Text('Arc Color ${index + 1}'),
      content: Wrap(spacing: 10, runSpacing: 10, children: colors.map((c) => GestureDetector(
        onTap: () {
          setState(() { if (index == 0) _arcColor1 = c; else if (index == 1) _arcColor2 = c; else _arcColor3 = c; });
  ...
  void _create() {
    if (_formKey.currentState!.validate()) {
      Navigator.pop(context, {
        'type': 'Gauge', 'label': _panelNameCtrl.text.trim(),
        ...
        'arcColor1': _arcColor1.value.toString(), 'arcColor2': _arcColor2.value.toString(),
        'arcColor3': _arcColor3.value.toString(), 'qos': _qos,
      });
    }
  }
