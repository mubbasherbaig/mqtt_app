...
class AddNodeStatusPanelScreen extends StatefulWidget {
  ...
  @override
  State<AddNodeStatusPanelScreen> createState() => _AddNodeStatusPanelScreenState();
}
...
class _AddNodeStatusPanelScreenState extends State<AddNodeStatusPanelScreen> {
  ...
  void dispose() {
    ...
    _payloadSyncRequestCtrl.dispose(); _payloadOnlineCtrl.dispose(); _payloadOfflineCtrl.dispose();
    super.dispose();
  }
  ...
  void _pickIconColor(bool isOnline, AppLocalizations l) {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Wrap(spacing: 10, runSpacing: 10, children: colors.map((c) => GestureDetector(
        onTap: () {
          setState(() { if (isOnline) _onlineIconColor = c; else _offlineIconColor = c; });
          Navigator.pop(context);
        },
        child: Container(width: 40, height: 40, decoration: BoxDecoration(color: c, shape: BoxShape.circle,
            border: Border.all(color: (isOnline ? _onlineIconColor : _offlineIconColor) == c ? Colors.black : Colors.transparent, width: 3))),
      )).toList()),
    ));
  }
