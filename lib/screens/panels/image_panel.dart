...
class _AddImagePanelScreenState extends State<AddImagePanelScreen> {
  ...
  final List<String> _imageSources = ['URL Payload', 'Base64 Payload', 'Binary Payload'];
  ...
  void _create() {
    if (_formKey.currentState!.validate()) {
      Navigator.pop(context, {
        'type': 'Image', 'label': _panelNameCtrl.text.trim(),
        'topic': _topicCtrl.text.trim(), 'imageSource': _imageSource,
        'autoRefresh': _autoRefresh, 'fitToPanelWidth': _fitToPanelWidth,
        'qos': _qos,
      });
    }
  }
