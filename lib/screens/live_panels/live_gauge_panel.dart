...
class LiveGaugePanel extends StatefulWidget {
  ...
  const LiveGaugePanel({super.key, required this.panel, required this.topic, required this.mqtt});
  @override
...
class _LiveGaugePanelState extends State<LiveGaugePanel> {
  ...
  double get _value => ((double.tryParse(_lastPayload) ?? _min) * _factor).clamp(_min, _max);
  double get _pct => _max == _min ? 0 : ((_value - _min) / (_max - _min)).clamp(0.0, 1.0);
  ...
  Widget build(BuildContext context) {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      SizedBox(
        width: 90, height: 55,
        child: CustomPaint(painter: _GaugePainter(_pct, _c('arcColor1', Colors.green), _c('arcColor2', Colors.yellow), _c('arcColor3', Colors.orange), _arcColor)),
      ),
      Text(_display, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: _arcColor)),
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('${_min.toStringAsFixed(_dec)}', style: const TextStyle(fontSize: 9, color: Colors.black45)),
        Text('${_max.toStringAsFixed(_dec)}', style: const TextStyle(fontSize: 9, color: Colors.black45)),
      ]),
    ]);
  }
}
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(cx, cy), Offset(nx, ny), Paint()..color = Colors.black54..strokeWidth = 2..strokeCap = StrokeCap.round);
