...
class _LiveColorPickerPanelState extends State<LiveColorPickerPanel> {
  ...
  void initState() {
    ...
    _unsub = widget.mqtt.subscribe(_subTopic, (payload) {
      ...
      try {
        setState(() => _color = _parseHexColor(extracted));
      } catch (_) {}
    });
  }
                ...
                (c) => GestureDetector(
                  onTap: () {
                    ...
                    if (widget.mqtt.isConnected) {
                      final jsonPattern =
                          widget.panel['jsonPattern'] as String? ?? '';
                      final toSend = buildJsonPayload(
                        _colorToHex(c),
                        jsonPattern,
                      );
                      widget.mqtt.publish(
      ...
      children: [
        GestureDetector(
          ...
          child: Container(
            ...
            child: ok
                ? const Icon(Icons.colorize, color: Colors.white, size: 22)
                : null,
          ),
        ),
        if (!_hideValue) ...[
